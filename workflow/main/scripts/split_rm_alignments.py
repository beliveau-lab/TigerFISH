...
script_name = "split_rm_alignments"
...
def split_file(probe_df,out_path):
    ...
    grouped_regions=probe_df.groupby('repeat_coords',sort=False)
    
    #write each repeat region into a seperate file
    for name,group in grouped_regions:
        
        group.to_csv(str(out_path) + "/" + str(name) + "_alignments.txt", header=False,
                     index=False, sep="\t")
