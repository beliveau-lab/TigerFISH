...
class SequenceCrawler:
    def __init__(self, input_string,fasta_scaffold,chr_name, l, L, gcPercent, GCPercent, nn_table, tm, TM,
                 ...
                 OverlapModeVal, verbocity, reportVal, debugVal, metaVal, out_path):
        ...
        """Change this into a generic seq object that expects a DNA string"""
        #for seq_record in Seq(self.input_string, generic_dna):
        #    self.block = str(seq_record).upper()
        #    print(self.block)

        self.block=str(Seq(self.input_string,generic_dna)).upper()
        #print(self.block)
    ...
    def getFrontVals(self, letter):
             ...
             if letter == 'G' or letter == 'C':
                 return (self.stackTable['init_G/C'][self.dH],self.stackTable['init_G/C'][self.dS])
             elif letter == 'A':
                 return (self.stackTable['init_A/T'][self.dH],self.stackTable['init_A/T'][self.dS])
             return (self.stackTable['init_A/T'][self.dH] + self.stackTable['init_5T/A'][self.dH],self.stackTable['init_A/T'][self.dS] + self.stackTable['init_5T/A'][self.dS])
    ...
    def resetTmVals(self, startInd, startLen):
          ...
          self.currdH = sum(self.hQueue[:startLen - 1])+ self.stackTable['init'][self.dH]+ self.frontH + self.backH
          self.currdS = sum(self.sQueue[:startLen - 1])+ self.stackTable['init'][self.dS]+ self.frontS + self.backS
    ...
    def probeCheck(self, seq5, ind, i, j):
         ...
         if self.Ncheckopt(seq5) == -1 and self.prohibitCheck(seq5) and self.tmCheck(seq5, ind, i, j) and self.gcCheck(seq5):
             return True
    ...
    def run(self):
         ...
         """
         Not needed because this modified version takes in a string and not a file object
         """
         ...
         if self.headerVal is None:
             ...
             if len(headerParse) != 1:
                 ...
                 self.start = 1
                 stop = len(self.block)
             elif 'range=' in headerParse:
                 chrom = headerParse.split('=')[1].split(':')[0]
                 #print(chrom)
                 self.start = int(str(headerParse).split(':')[1].split('-')[0])
                 stop = str(headerParse).split('-')[1].split(' ')[0]
        ...
        # Make a list to store candidate probe coordinates and sequences.
         ...
         while i < int(blockLen) - int(self.l):
             ...
             if self.seqCheck(self.block[i:i + self.l], i):
                 ...
                 j = 0
                 while i + j + self.l < int(blockLen) and j < sizeRange and not self.probeCheck(self.block[i:i + j + self.l],i, i, j):
                     j += 1
         ...
         with open(str(self.out_path), 'a+') as f:
         ...
         if self.metaVal:
             ...
             if probeNum == 0:
                 metaText.write('%s\t%s\t%d\t No candidate probes discovered' % (self.inputFile, Version, probeNum))
                 metaText.close()
             else:
                 metaText.write('%s\t%s\t%d\t%0.2f\t%0.2f' % (self.inputFile, Version, probeNum,
        ...
        # If desired, create report file.
         if self.reportVal:
             ...
             windowCount = (self.N_int_failCount + self.N_block_failCount + self.prohib_failCount + self.Tm_fail_lowCount + self.Tm_fail_highCount + self.gc_fail_lowCount+ self.gc_fail_highCount + probeNum)
             reportOut = open('%s_blockParse_log.txt' % chrom, 'w')
...
def runSequenceCrawler(input_string,fasta_scaffold,chr_name, l, L, gcPercent, GCPercent, nn_table, tm, TM, X, sal, form, sp, conc1, conc2, headerVal, bedVal,OverlapModeVal, verbocity, reportVal, debugVal, metaVal, out_path):
...
def main():
    """Runs the crawler through the given block sequence to identify probes within the FASTA file satisfying the constraints provided through the commandline interface."""
    startTime = timeit.default_timer()
    # Allow user to input parameters on command line.
    userInput = argparse.ArgumentParser(description='%s version %s. Requires a FASTA file as input. Currently, only '
